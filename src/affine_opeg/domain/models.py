...
class SamplingProgress(BaseModel):
    """One cell's progress row.

    Two counters with distinct semantics (see migration 0002):

    * ``attempts`` — claim-time counter. Each claim_next_cell bumps it
      atomically and hands the caller ``sample_idx = attempts - 1``.
      Identical to a "slots reserved" counter — includes failures.
    * ``collected`` — success counter. Bumped only when a status='ok'
      rollout is persisted, in the same transaction as the insert.

    A cell is "finished" when ``collected >= target_samples``; the
    publisher's frozen check also accepts ``attempts >= max_attempts``
    so heavily-failing cells eventually get published (or skipped) with
    whatever success rows they have.

    ``sample_idx`` (handed out at claim time) is the rollout's stable
    business key inside the cell. Successful and failed rollouts share
    the same sample_idx space, with a UNIQUE constraint on
    ``(env, task, teacher, sample_idx)`` preventing two writers from
